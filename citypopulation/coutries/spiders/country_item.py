...
class CountrySpider(scrapy.Spider):
	...
	def parse(self, response):
		for href in response.css("tr td[valign = 'top'] a"):
			...
			request = scrapy.Request(url, callback=self.parse_country)
			request.meta["contruy"] = href.xpath("text()").extract()[0]
			yield request
	...
	def parse_country(self, response):
		for href in response.css("tr td[valign = 'top'] a"):
			...
			request.meta["contruy"] = response.request.meta["contruy"]
			yield request
	...
	def parse_contry_infor(self,response):
		...
		info["Country"] = response.meta["contruy"]
		yield info